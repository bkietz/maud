...
- If there are globs on which configuration depends, then each build must check
  that those globs have not changed, which introduces overhead.
...
``MAUD_INCLUDE_DIRS``
    ...
    so for example ``$project_root/utils/include/evil/macros.h`` can be
    included by any source with ``#include "evil/macros.h"``.
